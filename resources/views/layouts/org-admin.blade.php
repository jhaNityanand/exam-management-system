...
<div id="panel-root"
    ...
    <aside id="app-sidebar"
           class="flex-shrink-0 bg-blue-900 text-white flex flex-col transition-all duration-200 {{ ($sidebarCollapsedSetting ?? false) ? 'w-16' : 'w-64' }}">
        <div class="flex items-center gap-3 px-4 py-5 border-b border-blue-800">
            <div class="w-8 h-8 rounded-lg bg-blue-400 flex items-center justify-center font-bold text-sm text-blue-900 flex-shrink-0">EX</div>
            <span class="font-semibold text-lg tracking-tight truncate" data-sidebar-label>ExamMS</span>
            <span class="ml-auto text-xs bg-blue-600 text-blue-100 px-2 py-0.5 rounded-full whitespace-nowrap hidden sm:inline" data-sidebar-label>Org Admin</span>
        </div>

        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1 flex flex-col">
            <div class="space-y-1 flex-1">
                @include('layouts.partials.org-admin-sidebar')
            </div>
            @include('layouts.partials.org-panel-footer')
        </nav>

        <div class="px-3 py-4 border-t border-blue-800 flex items-center gap-2">
            ...
            <div class="flex-1 min-w-0" data-sidebar-label>
                ...
                <p class="text-xs text-blue-300 truncate">Organization</p>
            </div>
            <form method="POST" action="{{ route('logout') }}" data-sidebar-label>
                @csrf
                <button type="submit" title="Logout" class="text-blue-300 hover:text-white transition-colors p-1">
