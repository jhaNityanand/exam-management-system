@php
    $topLinks = [
        ...
        [
            ...
            'parentRoute' => 'admin.blogs.index',
            'children' => [
                ['route' => 'admin.blogs.index', 'label' => 'Blogs'],
                ['route' => 'admin.blogs.categories.index', 'label' => 'Categories'],
            ]
        ],
        [
            'label' => 'News',
            'icon' => 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z',
            'parentRoute' => 'admin.news.index',
            'children' => [
                ['route' => 'admin.news.index', 'label' => 'News'],
                ['route' => 'admin.news.categories.index', 'label' => 'Categories'],
            ]
        ],
        [
...
@foreach ($topLinks as $link)
    @php
        ...
        if (isset($link['route'])) {
            try {
                $match = $link['match'] ?? (rtrim($link['route'], '.*') . '*');
                $active = ($link['route'] !== '#') && request()->routeIs($match);
            } catch (\Throwable) {
                $active = false;
            }
