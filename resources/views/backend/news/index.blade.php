    ...
    <section class="panel-card overflow-hidden">
        <div class="border-b border-slate-200/80 px-4 py-4 sm:px-6 dark:border-slate-800">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl font-semibold tracking-tight text-slate-950 dark:text-white">News Items</h2>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Create, publish, and manage news for your organization.</p>
                </div>
                <div class="shrink-0 flex items-center gap-2">
                    <button type="button" id="btn-refresh-news" class="blog-refresh-btn inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800" title="Refresh list" aria-label="Refresh news list">
                    ...
                    <a href="{{ route('admin.news.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 transition shadow-sm">
        ...
        <div class="relative overflow-x-auto min-h-[300px]" id="ajax-table-container">
                ...
                <thead class="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900/60 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                    <tr>
                        <th class="px-3 py-2.5 w-10">
                            <input type="checkbox" id="news-select-all" class="list-select-all rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" aria-label="Select all news items">
                        </th>
            ...
            <div id="news-empty" class="hidden py-12 text-center">
                <h3 class="text-sm font-semibold text-slate-900 dark:text-white">No news items found</h3>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Try adjusting filters or create a new post.</p>
                <div class="mt-5">
                    <a href="{{ route('admin.news.create') }}" class="panel-button-primary">Create News</a>
                </div>
            </div>
        </div>

        <div class="border-t border-slate-200/80 px-4 py-4 sm:px-6 dark:border-slate-800">
            <div id="news-pagination" class="flex items-center justify-between"></div>
        </div>
    </section>
</div>
