    ...
    <x-page-card class="category-builder-card overflow-visible relative z-10 w-full">
        <form action="{{ route('admin.questions.update', $question) }}" method="POST" id="question-form" enctype="multipart/form-data" class="category-builder">
            ...
            <div class="category-builder__header px-4 py-6 sm:px-6">
                <div>
                    <h1 class="category-builder__title tracking-tight text-slate-900">Edit Question</h1>
                    <p class="category-builder__subtitle text-slate-500">
                        Update the question details, options, answers, and explanations.
                    </p>
                </div>
            </div>
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="panel-input mt-1 block w-full">
                            @foreach(($questionTypes ?? \App\Support\ExamFormats::questionTypes()) as $type)
                                <option value="{{ $type['id'] }}" {{ old('type', $question->type) == $type['id'] ? 'selected' : '' }}>{{ $type['label'] }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 shrink-0 pt-2">
                    ...
                    <div class="lg:col-span-3">
                        ...
                        <select id="marks_type" name="marks_type" class="panel-input mt-1 block w-full">
                            <option value="single" {{ old('marks_type', $question->marks_type) == 'single' ? 'selected' : '' }}>Single Marks</option>
                            <option value="multiple" {{ old('marks_type', $question->marks_type) == 'multiple' ? 'selected' : '' }}>Multiple Marks</option>
                        </select>
                    ...
                    <div class="lg:col-span-6">
                        ...
                        <input type="hidden" name="marks" id="marks" value="{{ old('marks', $question->marks ?? 1) }}">
                        <select name="marks_list[]" id="marks_list" class="hidden" multiple>
                            @for ($i = 1; $i <= 10; $i++)
                                <option value="{{ $i }}" {{ in_array($i, old('marks_list', $question->marks_list ?? [])) ? 'selected' : '' }}>{{ $i }}</option>
                            @endfor
                        </select>
                    </div>
                </div>
                    ...
                    <div id="section-true-false" class="space-y-4 question-type-section hidden">
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 p-4 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition flex-1">
                                <input type="radio" name="tf_answer" value="True" class="w-5 h-5 text-indigo-600 focus:ring-indigo-500" {{ old('correct_answer', $question->correct_answer) == 'True' ? 'checked' : '' }}>
                                <span class="font-medium text-slate-900 dark:text-white ml-2">True</span>
                            </label>
                            <label class="flex items-center gap-2 p-4 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition flex-1">
                                <input type="radio" name="tf_answer" value="False" class="w-5 h-5 text-indigo-600 focus:ring-indigo-500" {{ old('correct_answer', $question->correct_answer) == 'False' ? 'checked' : '' }}>
...
@push('scripts')
    ...
    <script>
        window.existingQuestion = @json($question);
