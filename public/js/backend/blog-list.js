document.addEventListener('DOMContentLoaded', () => {
    ...
    const bulkBar = document.getElementById('blogs-bulk-bar');
    const selectAll = document.getElementById('blogs-select-all');
    const selectedCountEl = document.getElementById('blogs-selected-count');
    const bulkActionsActive = document.getElementById('blogs-bulk-actions-active');
    const bulkActionsBin = document.getElementById('blogs-bulk-actions-bin');
    ...
    const filterBadge = document.getElementById('blogs-filter-badge');
    let currentTrash = 'active';
    const selectedIds = new Set();

    const statusClass = (status) => `blog-status-badge--${status || 'draft'}`;
    ...
    const blogsTable = new AjaxTable({
        containerSelector: '#ajax-table-container',
        apiUrl: window.blogsApiUrl,
        tableBodySelector: '#blogs-table-body',
        paginationSelector: '#blogs-pagination',
        searchSelector: '#blogs-search',
        perPageSelector: '#blogs-per-page',
        filterDrawerSelector: '#filter-drawer',
        filterToggleSelector: '#btn-toggle-filters',
        filterDrawerFormSelector: '#filter-drawer-form',
        loadingSelector: '#blogs-loading',
        emptySelector: '#blogs-empty',
        skeletonColumns: 10,
        defaultSort: 'id',
        ...
        onFetchSuccess: () => {
            ...
            document.querySelectorAll('.blog-sort-btn').forEach((btn) => {
                const active = btn.dataset.sortKey === blogsTable.sort;
                btn.classList.toggle('is-active', active);
                btn.classList.toggle('is-asc', active && blogsTable.direction === 'asc');
                btn.classList.toggle('is-desc', active && blogsTable.direction === 'desc');
                btn.setAttribute('aria-sort', active ? (blogsTable.direction === 'asc' ? 'ascending' : 'descending') : 'none');
            });
        },
        rowTemplate: (item, index, meta) => {
            const showUrl = `${window.blogsIndexUrl}/${item.id}`;
            const editUrl = `${showUrl}/edit`;
            const page = Number(meta?.current_page || blogsTable.page || 1);
            const perPage = Number(meta?.per_page || blogsTable.per_page || 10);
            const total = Number(meta?.total ?? 0);
            const offset = (page - 1) * perPage + index;
            const serial = blogsTable.sort === 'id' && blogsTable.direction === 'desc'
            ...
            const tags = (item.tag_names || []).slice(0, 3).map((t) => `<span class="blog-tag-pill">${t}</span>`).join('');
            const moreTags = (item.tag_names || []).length > 3 ? `<span class="blog-tag-pill">+${item.tag_names.length - 3}</span>` : '';

            const isBin = currentTrash === 'bin';

            return `
                <tr class="blog-list-row list-row group">
                    ...
                    <td class="px-4 py-2.5 align-middle whitespace-nowrap">
                        <span class="blog-status-badge ${statusClass(item.status)}">${item.status_label || item.status}</span>
                    </td>
    ...
    const applyTrashFilter = (trash) => {
        syncTrashUi(trash);
        blogsTable.filters = { ...blogsTable.filters, trash };
        blogsTable.page = 1;
        blogsTable.fetch();
    };

    const originalFetch = blogsTable.fetch.bind(blogsTable);
    blogsTable.fetch = function patchedFetch() {
    ...
    document.querySelectorAll('.blog-sort-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (!key) return;
            if (blogsTable.sort === key) {
                blogsTable.direction = blogsTable.direction === 'asc' ? 'desc' : 'asc';
            } else {
                blogsTable.sort = key;
                blogsTable.direction = 'asc';
            }
            blogsTable.page = 1;
            blogsTable.fetch();
        });
    ...
    document.getElementById('btn-refresh-blogs')?.addEventListener('click', function onRefresh() {
        const btn = this;
        if (blogsTable.loading) return;
        btn.classList.add('is-refreshing');
        btn.disabled = true;
        blogsTable.page = 1;
        blogsTable.sort = blogsTable.defaultSort;
        blogsTable.direction = blogsTable.defaultDirection;
        blogsTable.fetch();
        const watch = setInterval(() => {
            if (!blogsTable.loading) {
                clearInterval(watch);
    ...
    document.getElementById('btn-bulk-delete')?.addEventListener('click', () => {
        Swal.fire({
            ...
            text: 'Selected blog posts will be moved to the bin.',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc2626',
            confirmButtonText: 'Yes, delete',
        }).then((result) => {
            if (result.isConfirmed) submitBulk('bulk-delete-form');
        });
    });

    document.getElementById('btn-bulk-restore')?.addEventListener('click', () => {
        submitBulk('bulk-restore-form');
    });

    document.getElementById('blogs-bulk-status')?.addEventListener('change', (event) => {
        if (!event.target.value) return;
        const form = document.getElementById('bulk-status-form');
        form.querySelector('[name="status"]').value = event.target.value;
        submitBulk('bulk-status-form');
    });
    ...
    tableBody?.addEventListener('click', (e) => {
        ...
        if (deleteBtn) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const form = document.getElementById('delete-blog-form');
                    form.action = `${window.blogsIndexUrl}/${id}`;
                    form.submit();
                }
            });
            return;
        ...
        if (restoreBtn) {
            ...
            const form = document.getElementById('restore-blog-form');
            form.action = `${window.blogsRestoreUrl}/${id}/restore`;
            form.submit();
        }
    });
