document.addEventListener('DOMContentLoaded', () => {
    ...
    const seoSlugPreview = document.getElementById('news-seo-slug');
    ...
    const baseUrl = window.newsBaseUrl || `${window.location.origin}/news`;
    ...
    if (tagsEl && window.TomSelect) {
        ...
        const tagsSelect = new TomSelect(tagsEl, {
            ...
            render: {
                item: (data, escape) => `<div class="item news-tag-item">${escape(data.text)}</div>`,
    ...
    const hydrateExistingMedia = (fieldRoot) => {
        const existing = window.newsExistingMedia || {};
