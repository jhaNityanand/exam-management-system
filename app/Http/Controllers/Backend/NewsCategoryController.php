...
use App\Http\Requests\Backend\NewsCategory\StoreNewsCategoryRequest;
use App\Http\Requests\Backend\NewsCategory\UpdateNewsCategoryRequest;
 ...
 *
 * Handles CRUD for News Categories. Categories are scoped to
 * the authenticated user's organization.
 *
 * Route prefix : admin/news/categories
 * Route names  : admin.news.categories.{index|create|store|edit|update|destroy}
 */
class NewsCategoryController extends Controller
{
    use HandlesCategoryListActions, ResolvesCurrentOrganization;

    protected function categoryModelClass(): string { return NewsCategory::class; }
    protected function categoryIndexRoute(): string { return 'admin.news.categories.index'; }
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = NewsCategory::forOrg($orgId);
            if ($trash === 'bin') {
                $query->onlyTrashed();
            }
            ...
            return view('backend.news-categories.partials.tree-list', compact('categories'))->render();
        }

        $categories = collect([]);
        return view('backend.news-categories.index', compact(
            'categories',
            'search',
            'status',
            'sort',
     ...
     */
    public function show(NewsCategory $category): \Illuminate\Http\JsonResponse
    {
        $orgId = $this->currentOrgId();
    ...
    public function store(StoreNewsCategoryRequest $request): RedirectResponse
    {
        ...
        return redirect()
            ...
            ->with('success', 'News category hierarchy saved successfully.');
    }
    ...
    public function edit(NewsCategory $category): View
    {
        ...
        return view('backend.news-categories.edit', compact('category'));
    }
    ...
    public function update(
        ...
        return redirect()
            ->route('admin.news.categories.index')
            ->with('success', 'News category hierarchy updated successfully.');
    }
    ...
    public function destroy(NewsCategory $category): RedirectResponse
    {
        ...
        return redirect()
            ->route('admin.news.categories.index')
            ->with('success', "News category \"{$category->name}\" deleted successfully.");
    }
}
